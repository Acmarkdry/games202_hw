 ...
 *
 */
vec3 EvalDiffuse(vec3 wi, vec3 wo, vec2 uv) { //bsdf的值
  vec3 L = vec3(0.0);
  vec3 albedo = GetGBufferDiffuse(uv);
  vec3 normal = GetGBufferNormalWorld(uv);
  float cos = max(0.0,dot(normal,wi));
  return albedo*cos*INV_PI; // 环境光和点乘
}
 ...
 *
 */
vec3 EvalDirectionalLight(vec2 uv) { // 着色点位于uv处得到的光源辐射度
  vec3 le = GetGBufferuShadow(uv)*uLightRadiance; // 可见性*光照强度
  return le;
}

bool RayMarch(vec3 ori, vec3 dir, out vec3 hitPos) {
  float step = 0.05;
  const int totalStopTimes = 150;
  int curStepTimes = 0;

  vec3 stepDir = normalize(dir)*step;
  vec3 curPos = ori;
  for(int curStepTimes = 0;curStepTimes < totalStopTimes;curStepTimes++){
...
void main() {
  ...
  vec3 w0 = normalize(uCameraPos - worldPos);

  // 直接光照
  L = EvalDiffuse(wi,w0,screenUV)*EvalDirectionalLight(screenUV); // 直接光照的效果
  // L = (GetGBufferDiffuse(screenUV) + EvalReflect(wi,w0,screenUV))/2.0; // 反射效果
  ...
  for(int i = 0;i < SAMPLE_NUM;i++){
    ...
    if(RayMarch(worldPos,dir,position_1)){
      ...
      L_ind += EvalDiffuse(dir,w0,screenUV)/pdf*EvalDiffuse(wi,dir,hitScreenUV)*EvalDirectionalLight(hitScreenUV);
    }
  }
